...
Dim wsInput As Worksheet
    ...
    ContinuousWide = 12 ' Width for Continuous outcome merged cells
    DichotomousWide = 9  ' Width for Dichotomous outcome merged cells
    ...
    With wsInput
        ...
        ' Clear the contents of the newly added row from startCol to lastCol_tr3
        Dim foundCell As Range
        Dim lastCol_tr3 As Long
        Set foundCell = wsInput.Rows(4).Find("treatment3")
        If Not foundCell Is Nothing Then
            lastCol_tr3 = foundCell.Column + foundCell.MergeArea.Columns.Count - 1
            .Range(.Cells(maxInputRow + 1, 2), .Cells(maxInputRow + 1, lastCol_tr3)).Clearcontents
        Else
            MsgBox "treatment3 not found."
            Exit Sub
        End If
...
Sub Insert_Continuous(name, j)
    ...
    With wsContinuous
        ...
        Set arms = Union(.Cells(6, 2), .Cells(6, 6), .Cells(6, 10))
        Set patients = Union(.Cells(6, 5), .Cells(6, 9), .Cells(6, 13))
        
        ' Get columns with strategies in wsInput
        Call FindStrategiesAndSetColumns(wsInput, startCol, lastCol)
        
        ' Insert formulas for arms
        armnum = startCol - 3 ' Column number for arm1 in Strategies
...
Sub Insert_Dichotomous(name, j)
    ...
    With wsDichotomous
        ...
        ' Autofill formulas to the last colored row
        .Range("B6:J6").AutoFill .Range("B6:J" & LastColoredRow), Type:=xlFillDefault
        
        ' Copy the results to wsInput
        .Range(.Cells(6, 2), .Cells(6, 2 + 8)).Copy wsInput.Cells(LastColoredRow, j)
    End With
End Sub

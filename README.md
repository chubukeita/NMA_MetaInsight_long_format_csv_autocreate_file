...
#### "Strategies" Tab
The second tab from the left in the Input Form (default titled "Strategies") allows for entering IndexA, IndexB, and sample sizes for each group in the study. It supports the input of studies comparing up to four groups, namely "treatment1", "treatment2", "treatment3" and "treatment4". The input form for treatment1 is vertically aligned with "IndexA1," "IndexB1," and "Patients (n1)" representing IndexA, IndexB, and sample size of the first group, respectively. The subscript numbers indicate to which group they belong. If any values are unknown, leave them blank. After entering "IndexA" and "IndexB",  clicking the "Update" button in the Information tab updates the treatment and also the names of each group (arm). If the total number of groups to be compared in the NMA exceeds 8, the name of "arm" can be changed after checking the "Enter the name of arm directly" check box. When a group is selected from the list and the "Update" button is pressed, the "arm" cell of "Strategies" in "InputSheet" turns blue, and the original formula is overwritten by pasting in the value. However, when this operation is performed, the group names are not reflected even if they are entered in "IndexA" and "IndexB" of the "Strategies" tab of the "Input Form" that is displayed when double-clicking a cell in the "InputSheet". On the other hand, the data of "Patient (n)" is reflected in the "InputSheet" when the input value is changed. Note that if the "Update" button is accidentally pressed with a check box checked, it can be undone by copying and pasting the blue cell columns that have not been filled with the color containing the formula into the blue cells that have been accidentally entered.

...
Sub Update_method()
    ...
    ' For ContinuousOutcome, the number of columns in the combined cell is 12
    ' For DichotomousOutcome, the number of columns in the combined cell is 9
...
Private Sub UpdateStrategies(ws As Worksheet)
    ...
    With ws
        ...
        For o = 1 To 3
            ...
            If Controls("chk" & .Cells(5, 22 + o).Value).Value = True Then
                .Cells(activeRow, 22 + o).Value = Me.Controls("txt" & .Cells(5, 22 + o).Value).Text
                .Cells(activeRow, 22 + o).Interior.Color = vbRed
            End If
            ' Deselect checkbox
            Controls("chk" & .Cells(5, 22 + o).Value).Value = False
    ...
    For m = 1 To 3
        With ws
            ...
            If Controls("chkout" & pageIndex & "sub1tr" & m).Value = True Then
                ...
                .Cells(ActiveCell.Row, p + 3).Interior.Color = vbRed
            End If
            ' Deselect checkbox
    ...
    For n = 1 To 3
        With ws
            ...
            If Controls("chkout" & pageIndex & "sub2tr" & n).Value = True Then
                ...
                .Cells(ActiveCell.Row, q + 2).Interior.Color = vbRed
            End If
            ' Deselect checkbox
...
Private Sub UserForm_Initialize()
    ...
    With wsInput
        kmax = .Cells(.Rows.Count, 2).End(xlUp).Row
    ...
    ' Array setting
    Dim ary_o As Variant, ary_t As Variant
    With wsOutcome_Format
        ary_o = .Range(.Cells(3, 3), .Cells(imax + 2, 3)).Value
        ary_t = .Range(.Cells(3, 2), .Cells(imax + 2, 2)).Value
    End With
    ...
    For i = 1 To imax
        ' Add page
        MultiPage1.Pages.Add ("outcome" & i & " (" & wsOutcome_Format.Cells(i + 2, 3) & ")")
        ...
        Call Outcomes_ToggleControlsVisibility(i, wsOutcome_Format.Cells(i + 2, 2).Value)
    Next i

    ' Set up a list of combo boxes on the various outcome pages
    For i = 1 To imax
        With Me.Controls("cmb" & i)
            .List = Application.Transpose(ary_o) ' Set array as list
            .ListIndex = i - 1
        End With
        
        With Me.Controls("cmbtype" & i)
            .List = Application.Transpose(ary_t) ' Set array as list
            .ListIndex = i - 1
        End With
    Next i
...
Sub Strategies_AddLabelsToPage(pageIndex As Long)
    ...
    With TableSheet
        ...
        controlNames = Array("maintitle", "tr1", "tr2", "tr3", _
                             .Cells(3, 3).Value & "tr1", .Cells(3, 3).Value & "tr2", .Cells(3, 3).Value & "tr3", _
                             .Cells(5, 1).Value & "tr1", .Cells(5, 1).Value & "tr2", .Cells(5, 1).Value & "tr3", _
                             wsInput.Cells(5, 23).Value, wsInput.Cells(5, 24).Value, wsInput.Cells(5, 25).Value, _
                             "ntr1", "ntr2", "ntr3")
    End With
...
Sub Strategies_AddTextBoxesToPage(pageIndex As Long)
    ...
    With TableSheet
        ' Create list of control names
        controlNames = Array(.Cells(3, 3).Value & "tr1", .Cells(3, 3).Value & "tr2", .Cells(3, 3).Value & "tr3", _
                             .Cells(5, 1).Value & "tr1", .Cells(5, 1).Value & "tr2", .Cells(5, 1).Value & "tr3", _
                             wsInput.Cells(5, 23).Value, wsInput.Cells(5, 24).Value, wsInput.Cells(5, 25).Value, _
                             "ntr1", "ntr2", "ntr3")
    End With
...
Sub Strategies_AddCheckBoxesToPage(pageIndex As Long)
    ...
    With wsInput
        ' Create list of control names
        controlNames = Array(.Cells(5, 23).Value, .Cells(5, 24).Value, .Cells(5, 25).Value)
    End With
    ...
    With TableSheet
        For i = 1 To 3
            ...
            End With
             ...
             ' arm label
            controlName = "lbl" & left(wsInput.Cells(5, 22 + i).Value, Len(wsInput.Cells(5, 22 + i).Value) - 1) & i
            With Me.Controls(controlName)
                    .caption = wsInput.Cells(5, 22 + i).Value ' caption
                    .Font.Size = 9 ' Size 9
                    .left = 10 + (i - 1) * 100 ' Distance from the left
        ...
        For i = 1 To 3
            ...
            ' arm textbox
            controlName = "txt" & left(wsInput.Cells(5, 22 + i).Value, Len(wsInput.Cells(5, 22 + i).Value) - 1) & i
            With Me.Controls(controlName)
                    .left = 10 + (i - 1) * 100 ' Distance from the left
        ...
        For i = 1 To 3
            'chk checkbox
            controlName = "chk" & left(wsInput.Cells(5, 22 + i).Value, Len(wsInput.Cells(5, 22 + i).Value) - 1) & i
            With Me.Controls(controlName)
...
Sub Outcomes_AddLabelsToPage(pageIndex As Long)
    ...
    ' Create list of control names
    controlNames = Array("maintitle", "subtitle1", "sub1tr1", "sub1tr2", "sub1tr3", _
                         "meantr1", "meantr2", "meantr3", "sdtr1", "sdtr2", "sdtr3", _
                         "sub1ntr1", "sub1ntr2", "sub1ntr3", "subtitle2", "sub2tr1", _
                         "sub2tr2", "sub2tr3", "eventtr1", "eventtr2", "eventtr3", _
                         "sub2ntr1", "sub2ntr2", "sub2ntr3")
...
Sub Outcomes_AddTextBoxesToPage(pageIndex As Long)
    ...
    ' Create list of control names
    controlNames = Array("meantr1", "meantr2", "meantr3", "sdtr1", "sdtr2", "sdtr3", _
                         "sub1ntr1", "sub1ntr2", "sub1ntr3", "eventtr1", "eventtr2", "eventtr3", _
                         "sub2ntr1", "sub2ntr2", "sub2ntr3")
...
Sub Outcomes_AddCheckBoxesToPage(pageIndex As Long)
    ...
    ' Create list of control names
    controlNames = Array("sub1tr1", "sub1tr2", "sub1tr3", "sub2tr1", "sub2tr2", "sub2tr3")
...
Private Sub UpdateStrategiesControlsFromSheet(ws As Worksheet)
    ...
    With ws
        ...
        For o = 1 To 3
            ...
            Me.Controls("txt" & .Cells(5, 22 + o).Value).Text = .Cells(activeRow, 22 + o)
            Me.Controls("chk" & .Cells(5, 22 + o).Value).Value = False
            r = r + 3
        Next o
        
        ' Define arms range
        Set arms = .Range(.Cells(activeRow, 23), .Cells(activeRow, 25))
        
        o = 1
        For Each arm In arms
            If arm <> "" Then
                Me.Controls("lbl" & "tr" & o).caption = "treatment" & o & " （" & arm & " arm）"
            ElseIf arm = "" Then
...
Private Sub UpdateContinuousControlsFromSheet(startCell As Range, pageIndex As Long)
    ...
    For Each arm In arms
        If arm <> "" Then
            Me.Controls("lblout" & pageIndex & "sub1tr" & m).caption = "treatment" & m & " （" & arm & " arm）"
        ElseIf arm = "" Then
...
Private Sub UpdateDichotomousControlsFromSheet(startCell As Range, pageIndex As Long)
    ...
    For Each arm In arms
        If arm <> "" Then
            Me.Controls("lblout" & pageIndex & "sub2tr" & n).caption = "treatment" & n & " （" & arm & " arm）"
        ElseIf arm = "" Then

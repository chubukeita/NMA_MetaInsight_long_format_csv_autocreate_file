...
Sub binding_Continuous(name As String)
    ...
    With wsContinuous
        ...
        ' Change cells filled with red color to no color
        ...
        armnum = startCol - 3 ' Get column number for arm1 in Strategies
...
Sub binding_Dichotomous(name As String)
    ...
    With wsDichotomous
        ...
        ' Copy results to wsInput
        .Range(.Cells(3, 2), .Cells(LastColoredRow, 2 + 8)).Copy wsInput.Cells(3, lastCol + 1)
...
Sub Clearcontents(Sheet As Worksheet)
    With Sheet
        ' Delete values
        .Range(.Cells(6, 2), .Cells(350, 13)).Clearcontents
    End With
End Sub
Sub Clear_Red_Interior(Sheet As Worksheet)
    Dim r As Range
    With Sheet
        ' Change cells filled with red color to no color
        For Each r In .Range(.Cells(6, 2), .Cells(350, 13))
